...
public class Vista extends JFrame {
	...
	public Vista() {
		...
		panelmostrartodo.setBounds(0, 0, 1292, 548);
		contentPane.add(panelmostrartodo);
		panelmostrartodo.setLayout(null);
		panelmostrartodo.setVisible(false);

		JLabel lblNewLabel_3_4 = new JLabel("Información de los Streamers");
		lblNewLabel_3_4.setHorizontalAlignment(SwingConstants.CENTER);
		lblNewLabel_3_4.setForeground(new Color(0, 0, 128));
		lblNewLabel_3_4.setFont(new Font("Tw Cen MT Condensed", Font.BOLD, 80));
		lblNewLabel_3_4.setBounds(0, 11, 1292, 99);
		panelmostrartodo.add(lblNewLabel_3_4);

		scrollPane = new JScrollPane();
		scrollPane.setBounds(10, 121, 1145, 416);
		panelmostrartodo.add(scrollPane);

		// Crear tabla con columnas especificadas
		// Crear tabla con columnas especificadas
		tabla = new JTable();
		tabla.setBorder(new LineBorder(new Color(255, 255, 255), 2));
		tabla.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION); // Permite seleccionar múltiples celdas
		tabla.setBackground(new Color(64, 0, 128)); // Fondo de la tabla
		tabla.setColumnSelectionAllowed(true); // Permite seleccionar columnas
		tabla.setCellSelectionEnabled(true); // Permite seleccionar celdas
		tabla.setForeground(new Color(255, 255, 255)); // Color de texto
		tabla.setFillsViewportHeight(true); // La tabla ocupa toda la vista
		tabla.setSurrendersFocusOnKeystroke(true); // Permite la selección con el teclado
		scrollPane.setViewportView(tabla);

		// Modelo de la tabla con 5 columnas
		String[] columnNames = {  "ID Creador", "Nombre_Creador", "País", "Temática", "Seguidores Totales",
		...
		// Estilo de la tabla
		tabla.setFont(new Font("Tahoma", Font.PLAIN, 10)); // Fuente de las celdas
		tabla.setRowHeight(25); // Altura de las filas
		tabla.setGridColor(new Color(200, 200, 200)); // Color de las líneas de la cuadrícula
		tabla.setSelectionBackground(new Color(255, 128, 0)); // Fondo al seleccionar
		tabla.setSelectionForeground(Color.WHITE); // Color del texto al seleccionar

		// Estilo de los nombres de las columnas (TableHeader)
		header = tabla.getTableHeader();
		header.setFont(new Font("Tahoma", Font.BOLD, 12)); // Fuente para el encabezado
		header.setForeground(Color.DARK_GRAY); // Color de texto del encabezado
		header.setBackground(Color.ORANGE); // Fondo naranja
		tabla.setTableHeader(header);

		// Evitar que la tabla sea editable
		tabla.setDefaultEditor(Object.class, null); // Desactivar el editor de celdas

		// Aquí no desactivamos la tabla completamente (como antes con
		// setEnabled(false))
		// Ya que queremos permitir la selección pero no la edición.

		// Hacer que la tabla sea solo de lectura, pero seleccionable
		tabla.setEnabled(true); // Esto asegura que se puede seleccionar pero no editar
		...
		panelMenu.setBounds(0, 0, 1292, 652);
		...
		lblMenu.setBounds(0, 0, 1292, 132);
		panelMenu.add(lblMenu);

		panelBotones = new JPanel();
		panelBotones.setBackground(Color.BLACK);
		panelBotones.setBounds(154, 156, 1001, 356);
